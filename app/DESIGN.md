...
### Navigation (상단 고정, sticky)

- **데스크톱**: 단일 바 — 로고(픽셀칩+Silkscreen) | 중앙 섹션 탭 `홈·도감·상점·기록` | 우측 잔액 칩·아바타.
- **모바일**: 2줄 — [로고 | 잔액·아바타] 윗줄 + [풀폭 4탭] 아랫줄. (하단 고정 탭바 아님 — 시안 FRAME 3 기준)
- 동일 컴포넌트(`auth-header.tsx`)의 반응형 분기(`lg:`)로 구현. 활성 탭은 하단 언더라인(primary).
