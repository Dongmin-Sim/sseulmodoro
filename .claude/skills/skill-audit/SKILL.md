---
name: skill-audit
description: Audits a Claude Code skill against the skill-authoring guide and DoD checklist, reporting pass/gaps by category. Runs on a review model in isolation. Use before committing a skill, to audit the whole set ("전체"), or when the user says "이 스킬 점검", "체크리스트 돌려", "가이드로 점검".
context: fork
...
- `all` / `전체` → audit every skill under `.claude/skills/`.
