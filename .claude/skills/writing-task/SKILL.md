...
## Create
```bash
python3 .claude/scripts/spec.py create task <TASK-N> title="..." milestone=<M-S-N> session=<BE|FE|DE>
```
Then fill 설명 and 완료조건 in the body with Edit (skeleton depth only — one-line purpose + rough conditions).

## Update (idempotent)
```bash
python3 .claude/scripts/spec.py set task <TASK-N> status=in-review pr=83
```
`set` patches only the given frontmatter keys and preserves the body. Keys: `status·branch·pr·start_date·end_date·milestone·session`.
