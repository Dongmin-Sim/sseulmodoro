...
def render_board(features, milestones, tasks, issues, metrics, events, source_mtime):
    generated_at = datetime.now().isoformat(timespec="seconds")
    lines = [
        "<!-- generated by reporting-status build_board.py — do not edit by hand -->",
    ...
    for f in features:
        mcount = sum(1 for m in milestones
                     if m["fields"].get("feature") == f["fields"].get("title"))
        lines.append(
            f"| {_cell(f['fields'].get('title'))} "
            f"| {_cell(f['fields'].get('status'))} | {mcount} |"
        )
    ...
    lines += ["", "## Metrics", "", "| metric | status |", "| --- | --- |"]
    for m in metrics:
        lines.append(
            f"| {_cell(m['fields'].get('title'))} | {_cell(m['fields'].get('status'))} |"
        )

    lines += ["", "## Events", "", "| event | status |", "| --- | --- |"]
    for e in events:
        lines.append(
            f"| {_cell(e['fields'].get('title'))} | {_cell(e['fields'].get('status'))} |"
        )
...
def main():
    ...
    if not workspace.is_dir():
        ...
        board_path.write_text(
            render_board([], [], [], [], [], [], 0.0), encoding="utf-8"
        )
    ...
    all_specs = features + milestones + tasks + issues + metrics + events
    ...
    board_path.write_text(
        render_board(features, milestones, tasks, issues, metrics, events, source_mtime),
        encoding="utf-8",
    )
    print(f"[build_board] wrote {board_path} "
          f"({len(features)} features, {len(milestones)} milestones, "
